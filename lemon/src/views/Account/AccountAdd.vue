...
export default {
  data() {
    // 验证用户名
    const userNamecheck = (rules,value,callback) => {
      if(value === ""){
        callback(new Error("用户名不能为空！"))
      }else if(value.length < 3 || value.length > 6){
        callback(new Error("用户名长度为3-6位"))
      }else{
        // 发送请求
        this.request.get('/account/checkuserName',{username:value})
          .then(res => {
    ...
    submitForm() {
      ...
      this.$refs.AccountAddForm.validate(valid => {
        ...
        if (valid) {
          ...
          // 发送axios请求
          console.log(this);
          this.request.post("/account/accountmanage", user)
            .then(data => {
